...
from tools.util import date_interpolation
        ...
        for it in range(0, bt['StormLon'][:, iS].shape[0], 1):
            if bt['Time'][it, iS] > 0:
                ...
                var, radius1, radius2 = date_interpolation(
                    datetime(1800, 1, 1)+timedelta(days=bt['Time'][it, iS]), PIVmax), 0, 500
                bt['PIwspdMean'][it, iS] = np.mean(
                    var[(distance <= radius2) & (distance >= radius1) & (var == var)])
                bt['PIwspd'][it, iS] = var[j0, i0]

                var, radius1, radius2 = date_interpolation(
                    datetime(1800, 1, 1)+timedelta(days=bt['Time'][it, iS]), u), 200, 800
                bt['UShearMean'][it, iS] = np.mean(
                    var[(distance <= radius2) & (distance >= radius1) & (var == var)])
                bt['UShear'][it, iS] = var[j0, i0]

                var, radius1, radius2 = date_interpolation(
                    datetime(1800, 1, 1)+timedelta(days=bt['Time'][it, iS]), v), 200, 800
                bt['VShearMean'][it, iS] = np.mean(
                    var[(distance <= radius2) & (distance >= radius1) & (var == var)])
                bt['VShear'][it, iS] = var[j0, i0]

                var, radius1, radius2 = date_interpolation(
                    datetime(1800, 1, 1)+timedelta(days=bt['Time'][it, iS]), meanrh), 200, 800
...
def getSpeedDir(iiS, block_id=None):
    ...
    if (bt['StormLat'][:, iS] == bt['StormLat'][:, iS]).any():
        ...
        it2 = np.argwhere(bt['StormLat'][:, iS] ==
                          bt['StormLat'][:, iS])[-1, 0]
        if it2 - it1 >= 2:
...
class mybt:
    def __init__(self, bt_array):
        for iv in list(bt_array.keys()):
            if 'Time' not in iv:
                setattr(self, iv, bt_array[iv])
            else:
                start = pd.Timestamp('1800-01-01').to_pydatetime()
                setattr(self, iv, start +
                        np.int_(bt_array[iv]*24)*timedelta(hours=1))
...
result_w, meanX_w, meanY_w, stdX_w, stdY_w, result_l, meanX_l, meanY_l, stdX_l, stdY_l = calCoefficient_water_guess(
    iy1, iy2+1, 12)
