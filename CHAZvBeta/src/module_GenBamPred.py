...
from . import calWindCov as calWindCov
...
def randomSeeding(iy):
    ...
    climInitLat = []
    climInitDate = []
    a = np.arange(gv.lldmask.size).reshape(
        gv.lldmask.shape[0], gv.lldmask.shape[1])
    llon1, llat1 = np.meshgrid(gv.llon, gv.llat)
    dummy = np.random.choice(a[(gv.lldmask == 0) & (
        np.abs(llat1) <= 65) & (np.abs(llat1) >= 3)], gv.seedN)
    y = dummy/a.shape[1]
    x = np.mod(dummy, a.shape[1])
    yeardays = np.array([calendar.monthrange(iy, im)[1]
                        for im in range(1, 13)]).sum()
    a = np.arange(0, yeardays, 1)
    days = np.random.choice(a, gv.seedN)
    date = pd.date_range(str(iy)+'-01-01', periods=365 +
                         calendar.isleap(iy)*1)[days]
    for idd in range(date.size):
        ...
        climInitDate.append(datetime.combine(
            date[idd].date(), datetime.min.time()))
...
def TCgiSeeding(gxlon, gxlat, gi, climInitLon, climInitLat, climInitDate, ratio, iy):
    ...
    for im in range(0, 12, 1):
        ...
        iday = np.random.choice(np.arange(calendar.monthrange(
            iy, im+1)[1]), size=np.int(np.rint(pk.sum()*ratio)), replace=True)
        if iday.size > 0:
...
def getTrackPrediction(u250, v250, u850, v850, dt, fstLon, fstLat, fstDate):
    ...
    vTrack = alpha*v850+(1.-alpha)*v250+vBeta * \
        np.sign(np.sin(fstLat*np.pi/180.))
    dx = uTrack*dt
    dy = vTrack*dt
...
def bam(iS, block_id=None):
    ...
    while fstDate < endDate:
        ...
        u250 = u250m2d[iy1:iy2+1, ix1:ix2+1] + \
            A[0, iy1:iy2+1, ix1:ix2+1]*F[iit, 0]
        v250 = v250m2d[iy1:iy2+1, ix1:ix2+1]+A[1, iy1:iy2+1,
                                               ix1:ix2+1]*F[iit, 0]+A[2, iy1:iy2+1, ix1:ix2+1]*F[iit, 1]
        u850 = u850m2d[iy1:iy2+1, ix1:ix2+1]+A[3, iy1:iy2+1, ix1:ix2+1]*F[iit, 0] +\
            A[4, iy1:iy2+1, ix1:ix2+1]*F[iit, 1] + \
            A[5, iy1:iy2+1, ix1:ix2+1]*F[iit, 2]
        ...
        fstLon, fstLat, fstDate = getTrackPrediction(
            u250, v250, u850, v850, dt, fstLon, fstLat, fstDate)
        ...
        fstldmask[count, b] = np.rint(
            np.nanmean(gv.ldmask[iy1:iy2+1, ix1:ix2+1]))
...
class fst2bt(object):
    ...
    def __init__(self, data):
        ...
        for iS in range(data['Time'].shape[1]):
            data['Time'][:, iS] = np.array(
                [datetime(1800, 1, 1, 0) if v is None else v for v in data['Time'][:, iS]])
        data['Time'][data['lon'] != data['lon']] = datetime(1800, 1, 1, 0)
        data['lon'][data['Time'] == datetime(1800, 1, 1, 0)] = float('Nan')
        self.StormLon = data['lon']
        self.StormLat = data['lat']
        self.Time = data['Time']
        self.PIwspd = np.full(data['lon'].shape, np.nan, dtype=float)
        self.PIslp = np.full(data['lon'].shape, np.nan, dtype=float)
        self.PIwspdMean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.dPIwspd = np.full(data['lon'].shape, np.nan, dtype=float)
        self.PIslpMean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.UShearMean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.UShear = np.full(data['lon'].shape, np.nan, dtype=float)
        self.VShearMean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.VShear = np.full(data['lon'].shape, np.nan, dtype=float)
        self.div200Mean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.div200 = np.full(data['lon'].shape, np.nan, dtype=float)
        self.T200Mean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.T200 = np.full(data['lon'].shape, np.nan, dtype=float)
        self.rh500_300Mean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.rh500_300 = np.full(data['lon'].shape, np.nan, dtype=float)
        self.rhMean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.rh = np.full(data['lon'].shape, np.nan, dtype=float)
        self.T100Mean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.T100 = np.full(data['lon'].shape, np.nan, dtype=float)
        self.dThetaEMean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.dThetaE = np.full(data['lon'].shape, np.nan, dtype=float)
        self.dThetaEs = np.full(data['lon'].shape, np.nan, dtype=float)
        self.dThetaEsMean = np.full(data['lon'].shape, np.nan, dtype=float)
        self.landmask = data['ldmask']
        self.landmaskMean = data['ldmask']
        self.trSpeed = np.zeros(data['lon'].shape)*float('Nan')
        self.trDir = np.zeros(data['lon'].shape)*float('Nan')
...
def get_predictors(iiS, block_id=None):
    ...
    predictors = ['StormMwspd', 'dVdt', 'trSpeed', 'dPIwspd', 'SHRD',
                  'rhMean', 'dPIwspd2', 'dPIwspd3', 'dVdt2', 'landmaskMean']
    ...
    for it in range(0, bt.StormLon[:, iS].shape[0], 1):
        if bt.Time[it, iS] != datetime(1800, 1, 1, 0):
            ...
            var, radius1, radius2 = date_interpolation(
                bt.Time[it, iS], PIVmax), 0, 500
            bt.PIwspdMean[it, iS] = np.mean(
                var[(distance <= radius2) & (distance >= radius1) & (var == var)])
            bt.PIwspd[it, iS] = var[j0, i0]

            var, radius1, radius2 = date_interpolation(
                bt.Time[it, iS], u), 200, 800
            bt.UShearMean[it, iS] = np.mean(
                var[(distance <= radius2) & (distance >= radius1) & (var == var)])
            bt.UShear[it, iS] = var[j0, i0]

            var, radius1, radius2 = date_interpolation(
                bt.Time[it, iS], v), 200, 800
            bt.VShearMean[it, iS] = np.mean(
                var[(distance <= radius2) & (distance >= radius1) & (var == var)])
            bt.VShear[it, iS] = var[j0, i0]

            var, radius1, radius2 = date_interpolation(
                bt.Time[it, iS], meanrh), 200, 800
            bt.rhMean[it, iS] = np.mean(
                var[(distance <= radius2) & (distance >= radius1) & (var == var)])
...
def get_seeding_ratio(fpath, iy1, iy2, EXP):
    ...
    NS = np.argwhere((bt_obs['StormYear'][:] >= iy1) & (
        bt_obs['StormYear'][:] <= iy2))[:, 0].shape[0]
    totalNS = float(NS)/gv.survivalrate
    ratio = 1.0
    # I mad mistake to make it 2 for most of CMIP5 runs, and need to fix that
    for iratio in range(0, 1):
        ## seeding storms ####
        time1 = time.time()
        climInitLon = []
        climInitLat = []
        ...
        for iy in range(iy1, iy2+1, 1):
            ...
            climInitLon, climInitLat, climInitDate = \
                TCgiSeeding(gxlon, gxlat, gi, climInitLon,
                            climInitLat, climInitDate, ratio, iy)
        NS = len(climInitLon)
        # print NS, totalNS,ratio
        ratio = float(totalNS)/float(NS)
    return ratio
...
def getBam(cDate, cLon, cLat, iy, ichaz, exp):
    ...
    diS = da.from_array(np.arange(0, nS, 1).astype(
        dtype=np.int32), chunks=(1,))
    ...
    for iS in niS:
        fsttime[iT1[iS]:iT2[iS]+1, iS] = \
            [climInitDate[iS]+timedelta(hours=6*iit)
             for iit in range(iT1[iS], iT2[iS]+1, 1)]
        fst = {'lon': fstlon, 'lat': fstlat,
               'Time': fsttime, 'ldmask': fstldmask}
