...
def resize_basinData():
    ...
    for iib in range(0, len(basinName), 1):
        ...
        f = gv.ipath + 'bt_'+ib+'.nc'
        bt1 = xr.open_dataset(f)
        for iv in list(bt1.variables.keys()):
            if iib == 0:
                if np.size(bt1.variables[iv].shape) > 1:
                    bt[iv] = np.zeros(
                        [maxCol, bt1.variables[iv].shape[1]])*float('nan')
                    bt[iv][:bt1.variables[iv].shape[0],
                           :] = bt1.variables[iv].values
                else:
                ...
                if np.size(bt1.variables[iv].shape) > 1:
                    dummy = np.zeros(
                        [maxCol, bt1.variables[iv].shape[1]])*float('nan')
                    dummy[:bt1.variables[iv].shape[0],
                          :] = bt1.variables[iv].values
        ...
        del bt1
    for iv in list(bt.keys()):
        if ((np.size(bt[iv].shape) > 1) and ('Time' not in iv)):
            bt[iv][bt['Time'] == 0] = float('nan')
...
def TCgiSeeding(gxlon, gxlat, gi, climInitLon, climInitLat, climInitDate, ratio):
    ...
    for im in range(0, 12, 1):
        ...
        iday = np.random.choice(np.arange(calendar.monthrange(
            iy, im+1)[1]), size=np.round(pk.sum()), replace=True)
        if iday.size > 0:
        ...
        if (fstDate.year != year0):
            ...
            if os.path.isfile(fileName):
                ...
                if fstDate.day != day0:
                    ...
                    FileAName =\
                        gv.ipath+'A_'+str(fstDate.year) + \
                        int2str(fstDate.month, 2)+'.nc'
                ...
                fstLon, fstLat, fstDate = getTrackPrediction(
                    u250, v250, u850, v850, dt, fstLon, fstLat, fstDate, uBeta, vBeta)
...
def get_predictors(iiS, block_id=None):
    ...
    for it in range(0, bt.StormLon[:, iS].shape[0], 1):
        if bt.Time[it, iS] > datetime(1800, 1, 1, 0):
            ...
            var, radius1, radius2 = date_interpolation(
                bt.Time[it, iS], meanrh1), 200, 800
            bt.rh500_300Mean[it, iS] = np.mean(
                var[(distance <= radius2) & (distance >= radius1) & (var == var)])
...
def calCoefficient_water_guess(ty1, ty2, ih):
    ...
    predictors = ['StormMwspd', 'dVdt', 'trSpeed', 'dPIwspd',
                  'SHRD', 'rhMean', 'dPIwspd2', 'dPIwspd3', 'dVdt2']
