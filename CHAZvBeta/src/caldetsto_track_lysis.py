...
def get_determin(iiS, block_id=None):
    ...
    TimeDepends = ['dThetaEsMean', 'T200Mean',
                   'rhMean', 'rh500_300Mean', 'div200Mean']
    if ((dummy.any()) and (np.abs(bt.StormLat[0, iS]) >= 5.)):
        it1 = 0
        it2 = np.min([np.argwhere(bt.StormLon[:, iS] == dummy[-1])
                     [-1, 0], bt.StormLon.shape[0]-5])
        if ((bt.StormLat[0, iS] >= 0) and (bt.StormLon[0, iS] >= 120) and (bt.StormLon[0, iS] <= 180)):
            bt.determin[0, iS] = np.max(
                [20, random.choice(intV[intV == intV])])  # kts
        else:
            bt.determin[0, iS] = np.max(
                [25, random.choice(intV[intV == intV])])  # kts
        ...
        for it in range(it1, it2+2, 2):
            if ((it+lT < bt.StormLon.shape[0]) and (bt.StormLon[it, iS] == bt.StormLon[it, iS])
                ...
                h1, v1 = reg4.getPrediction_v0input_result(bt, meanX, meanY, stdX, stdY, it,
                                                           iS, [
                                                               ih], result, predictors,
                                                           TimeDepends, v0, dvdt)
                v1 = v1[1]
                if v1 < 10:
                    v1 = float('Nan')
        ...
        for iit in range(it1+1, np.min([it2+1, bt.StormLon.shape[0]-2]), 2):
            ...
            if a == a:
                bt.stochastic[iit, iS, iNN] = \
                    0.5*(bt.stochastic[iit-1, iS, iNN] +
                         bt.stochastic[iit+1, iS, iNN])
...
# f.close()
    ...
    NS = np.argwhere((bt2['StormYear'][:] >= 1981) &
                     (bt2['StormYear'][:] <= 2012))[:, 0]
...
#    f.close()
    ...
    bt.__dict__['determin'] = np.zeros(bt.StormLon.shape)*float('nan')
    bt.__dict__['stochastic'] = np.zeros(
        [bt.StormLon.shape[0], bt.StormLon.shape[1], nsto])*float('nan')
    bt.__dict__['error'] = np.zeros(
        [bt.StormLon.shape[0], bt.StormLon.shape[1], nsto])*float('nan')
   ...
   # with open (gv.output_path+'bt_stochastic_det'+int2str(iy,4)+'_ens'+int2str(ichaz,3)+'.pik','w') as f:
    ...
    for iens in range(0, 1):
        ...
        if count1 == 0:
            ...
            for iv in dir(bt):
                if (('__' not in iv) and ('Time' not in iv) and (getattr(bt, iv).ndim == 2)):
                    b = np.zeros(
                        [maxCol, getattr(bt, iv).shape[1]])*float('nan')
                    b[0:getattr(bt, iv).shape[0], :] = getattr(bt, iv)
                    setattr(bt, iv, b)
                elif (('__' not in iv) and ('Time' not in iv) and ('timeY' not in iv) and (getattr(bt, iv).ndim == 3)):
                    b = np.zeros([maxCol, getattr(bt, iv).shape[1],
                                 getattr(bt, iv).shape[2]])*float('nan')
                    b[0:getattr(bt, iv).shape[0], :, :] = getattr(bt, iv)
                    setattr(bt, iv, b)
                elif 'Time' in iv:
                    b = np.empty(
                        [maxCol, getattr(bt, iv).shape[1]], dtype=object)
        ...
        bt1.stochastic[id1, iS1, :] = float('nan')
        ...
        iS1, iN1 = np.where((((basin0 < 5) & (basin2 >= 5)) | ((basin0 >= 5) & (basin2 < 5)) |
                             ((basin0 == 0) & (basin2 == 1))))
        bt1.stochastic[:, iS1, iN1] = float('nan')
        ...
        bt1.stochastic[bt1.stochastic == 0] = float('nan')
        ...
        bt1.stochastic[:, iS1, :] = float('nan')
        bt1.StormLon[0, iS1] = float('nan')
        ...
        bt1.stochastic[:, iS1, iN1] = float('nan')
        # bt1.stochastic[:,basin!=1,:] = float('nan')
        ...
        ds = xr.Dataset({
            ...
            'Mwspd': xr.DataArray(
                ...
                coords={'ensembleNum': ensembleNum,
                        'lifelength': lifelength, 'stormID': stormID},
                attrs={
                    '_FillValue': float('nan'),
                    'units': 'kt'
                }
            ...
            'time': xr.DataArray(
                data=date2num(
                    newdatenum, units='days since 1950-01-01 00:00', calendar='standard'),
                dims=['lifelength', 'stormID'],
                coords={'lifelength': lifelength, 'stormID': stormID},
                attrs={
        ...
        file_name = gv.output_path+gv.Model+'_' + \
            int2str(iy, 4)+'_ens'+int2str(iens, 3)+'.nc'
        print(file_name)
