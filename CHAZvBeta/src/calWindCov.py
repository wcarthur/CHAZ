...
def createNetCDF(covMatrix, iy, xlong, xlat):
    ...
    nc = Dataset(gv.pre_path+'Cov_'+int2str(iy, 4) +
    ...
    lats = nc.createVariable(
        'latitude', np.dtype('float32').char, ('latitude',))
    lons = nc.createVariable('longitude', np.dtype(
    ...
    lats[:] = xlat
    lons[:] = xlong
    months[:] = list(range(1, covMatrix.shape[1]+1, 1))
    ...
    for iv in range(len(var)):
        for iiv in range(iv, len(var), 1):
            ...
            # print vname
            var1 = nc.createVariable(vname, np.dtype(
                'float32').char, ('month', 'latitude', 'longitude'))
            var1.units = '(ms-1)^2'
...
def my_cov(x, y, naxis):
    n = x.shape[naxis]
    cov_bias = np.mean(x*y, axis=naxis) - \
        (np.mean(x, axis=naxis)*np.mean(y, axis=naxis))
...
def run_windCov():
    ...
    modely1m = np.int_([df_sub_uam[i].split('/')[-1].split('_')[-1][:4]
                       for i in range(df_sub_uam.shape[0])])
    modely2m = np.int_([df_sub_uam[i].split('/')[-1].split('_')
                       [-1].split('-')[-1][:4] for i in range(df_sub_uam.shape[0])])

    # reading daily data through Lamont URL
    # daily_csv should be in the namelist.py
    modely1d = np.int_([df_sub_vad[i].split('/')[-1].split('_')[-2][:4]
                       for i in range(df_sub_vad.shape[0])])
    modely2d = np.int_([df_sub_vad[i].split('/')[-1].split('_')[-2][:4]
                       for i in range(df_sub_vad.shape[0])])
    ...
    for iy in range(gv.Year1, gv.Year2+1):
        ...
        if arg_yd[0] != arg_y1:
            ...
            dsvadtime = pd.to_datetime(np.arange(
                ds_vad.day.shape[0]), unit='D', origin=pd.Timestamp(str(iy)+'-01-01'))
        ...
        # roughly middle of the month
        m = np.argwhere(date_daily.day == 15).ravel()
        ...
        f = interp1d(m, ua250m, bounds_error=False,
                     fill_value="extrapolate", axis=0)
        ua250md = f(d)
        f = interp1d(m, va250m, bounds_error=False,
                     fill_value="extrapolate", axis=0)
        va250md = f(d)
        f = interp1d(m, ua850m, bounds_error=False,
                     fill_value="extrapolate", axis=0)
        ua850md = f(d)
        f = interp1d(m, va850m, bounds_error=False,
                     fill_value="extrapolate", axis=0)
        va850md = f(d)
        # now we are going to do the monthly cov.
        covMatrix = np.zeros([10, 12, va250md.shape[1], va250md.shape[2]])
        for im in range(1, 13):
            arg_td = np.argwhere((dsvadtime.year == iy) &
                                 (dsvadtime.month == im)).ravel()
            ...
            arg_md = np.argwhere((dsvadtime[arg_td].year == iy) & (
                dsvadtime[arg_td].month == im)).ravel()
            u250p = ua250d-ua250md[arg_md]
            u250p2D = u250p.reshape(
                [u250p.shape[0], u250p.shape[1]*u250p.shape[2]])
            u850p = ua850d-ua850md[arg_md]
            u850p2D = u850p.reshape(
                [u250p.shape[0], u250p.shape[1]*u250p.shape[2]])
            v250p = va250d-va250md[arg_md]
            v250p2D = v250p.reshape(
                [u250p.shape[0], u250p.shape[1]*u250p.shape[2]])
            v850p = va850d-va850md[arg_md]
            v850p2D = v850p.reshape(
                [u250p.shape[0], u250p.shape[1]*u250p.shape[2]])
            ...
            for iv in range(len(var)):
                ...
                for iiv in range(iv, len(var), 1):
                    ...
                    covMatrix[count, im-1, :, :] = \
                        my_cov(eval(vname), eval(vname1), 0).reshape(
                            [u250p.shape[1], u250p.shape[2]])
