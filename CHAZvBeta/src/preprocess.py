...
def createNetCDF(covMatrix, iy, xlong, xlat):
    ...
    nc = Dataset(gv.pre_path+'Cov1_'+int2str(iy, 4) +
...
def run_preProcesses():
    ...
    il1 = np.int_(il1)
    il2 = np.int_(il2)
    # periodical BL, so 360 is equal to 0 degree.
    il2[il2 == olon.shape[0]] = 0
    jl1[jl1 < 0] = 0
    ...
    if True:
        for iy in range(y1, y2+1):
            ...
            covMatrix = np.zeros(
                [10, nc.variables['month'].shape[0], xlat.shape[0], xlon.shape[0]])
            count = 0
            for iv in list(nc.variables.keys())[3:]:
                var = nc.variables[iv][:]
                for im in range(var.shape[0]):
                    covMatrix[count, im] =\
                        ...
                        ((ollon[jl2, il2]-ollon[jl2, il1])
                         * (ollat[jl2, il2]-ollat[jl1, il2]))
                count += 1
    ...
    for iy in range(y1, y2+1):
        ...
        Longitude = nc.createVariable(
            'Longitude', np.dtype('float32').char, ('Longitude'))
...
def createNetCDF_A(A1, iy, im, xllon, xllat):
    ...
    """
    nc = Dataset(gv.pre_path+'A_'+int2str(iy, 4)+int2str(im, 2) +
                 '.nc', 'w', format='NETCDF3_CLASSIC')
    ...
    lats[:] = xllat
    lons[:] = xllon
    days[:] = list(range(0, A1.shape[1], 1))
    Adim[:] = list(range(0, A1.shape[0], 1))
    AA = nc.createVariable('A', np.dtype('float32').char,
                           ('Adim', 'days', 'latitude', 'longitude'))
    AA.units = ''
    AA[:] = A1
    nc.close()
    return ()
...
def run_preProcesses_A():
    ...
    fileName1 = gv.ipath+gv.PImodelname+'_' + \
        EXP+'_ENS'+ens[1]+'_'+str(y1)+'.mat'
    ...
    for iy in range(y1, y2+1):
        ...
        for im in range(1, 13):
            ...
            iv = list(nc.variables.keys())[4]
            var = nc.variables[iv][:]
            A = np.zeros([var.shape[0], var.shape[1],
                         xlat.shape[0], xlon.shape[1]])
            for imm in range(var.shape[0]):
                for inn in range(var.shape[1]):
                    A[imm, inn,] =\
                        ...
                        ((llon[jl2, il2]-llon[jl2, il1])
                         * (llat[jl2, il2]-llat[jl1, il2]))
            nc.close()
