...
import numpy as np
import random
from tools.util import find_range
...
def get_E1(bt):
    ...
    NS = np.argwhere((bt['StormYear'][:] >= 1981) &
                     (bt['StormYear'][:] <= 2012))[:, 0]
    # data = bt['errors'][:,NS,0]
    # data2 = bt.trueY[:,NS,0]
    data = bt['errors'][:, NS]
...
def get_E1_delta1(bt):
    ...
    for iC in np.arange(v0delta.min(), v0delta.max(), 10):
        if v0delta[v0delta < iC].shape[0] > 50:
            c1 = iC
            break
...
def get_mean(meanFile, predictors):
    ...
    n2 = len(predictors)
    meanX = np.full([len(hour), n2], np.nan, dtype=float)
    meanY = np.full([len(hour)], np.nan, dtype=float)
    stdX = np.full([len(hour), n2], np.nan, dtype=float)
    stdY = np.full([len(hour)], np.nan, dtype=float)
    f = open(meanFile)
    b = f.readlines()
    f.close()
    count1 = 0
