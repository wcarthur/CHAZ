...
class bstinfo(object):
    ...
    def __init__(self, ncFileName):
        ...
        self.PIwspd = np.full(yyyy.shape, np.nan, dtype=float)
        self.PIslp = np.full(yyyy.shape, np.nan, dtype=float)
        self.PIwspdMean = np.full(yyyy.shape, np.nan, dtype=float)
        self.dPIwspd = np.full(yyyy.shape, np.nan, dtype=float)
        self.PIslpMean = np.full(yyyy.shape, np.nan, dtype=float)
        self.UShearMean = np.full(yyyy.shape, np.nan, dtype=float)
        self.UShear = np.full(yyyy.shape, np.nan, dtype=float)
        self.VShearMean = np.full(yyyy.shape, np.nan, dtype=float)
        self.VShear = np.full(yyyy.shape, np.nan, dtype=float)
        self.div200Mean = np.full(yyyy.shape, np.nan, dtype=float)
        self.div200 = np.full(yyyy.shape, np.nan, dtype=float)
        self.T200Mean = np.full(yyyy.shape, np.nan, dtype=float)
        self.T200 = np.full(yyyy.shape, np.nan, dtype=float)
        self.rh500_300Mean = np.full(yyyy.shape, np.nan, dtype=float)
        self.rh500_300 = np.full(yyyy.shape, np.nan, dtype=float)
        self.rhMean = np.full(yyyy.shape, np.nan, dtype=float)
        self.rh = np.full(yyyy.shape, np.nan, dtype=float)
        self.T100Mean = np.full(yyyy.shape, np.nan, dtype=float)
        self.T100 = np.full(yyyy.shape, np.nan, dtype=float)
        self.dThetaEMean = np.full(yyyy.shape, np.nan, dtype=float)
        self.dThetaE = np.full(yyyy.shape, np.nan, dtype=float)
        self.dThetaEsMean = np.full(yyyy.shape, np.nan, dtype=float)
        self.dThetaEs = np.full(yyyy.shape, np.nan, dtype=float)
        self.landmask = np.full(yyyy.shape, np.nan, dtype=float)
        self.landmaskMean = np.full(yyyy.shape, np.nan, dtype=float)
        ...
        for ix in range(0, yyyy.shape[0], 1):
            for iy in range(0, yyyy.shape[1], 1):
                if ((dummy[ix, iy] == dummy[ix, iy]) and (dd[ix, iy] != 0)):
                    self.Time[ix, iy] = datetime(int(yyyy[ix, iy]), int(
                        mm[ix, iy]), int(dd[ix, iy]), int(hh[ix, iy]))
...
class read_ibtracs(object):
    ...
    def __init__(self, ncFileName, basins):
        ...
        for i in range(nNaN.shape[0]):
            year[i] = (datetime(1858, 11, 17, 0, 0) +
                       timedelta(days=np.array(days)[0, i])).year
            month[i] = (datetime(1858, 11, 17, 0, 0) +
                        timedelta(days=np.array(days)[0, i])).month
        ...
        wspd[wspd == -9990.] = float('nan')
        lon[lon == -30000.] = float('nan')
        lat[lat == -30000.] = float('nan')
...
class read_ibtracs_v4(object):
    ...
    def __init__(self, ncFileName, basins, gap):
        ...
        lat = np.array(lat[:, arg])
        year = year[arg]
        wspd = float_(np.array(wspd[:, arg]))
        days = np.array(days[:, arg])
        dist2land = np.array(dist2land[:, arg])
        wspd[wspd == -9999.] = float('nan')
        lon[lon == -9999.] = float('nan')
        lat[lat == -9999.] = float('nan')
        stormID = np.array(stormID[arg])
        names = np.array(names[:, arg])
