...
def date_interpolation(dateInput, fileInput):
    ...
    if dateInput.day >= 15:
        if dateInput.month < 12:
            ...
            fileOutput = fileInput[dateInput.month - 1, :, :] * (1 - ratio) + fileInput[
                dateInput.month, :, :
            ] * (ratio)
        else:
        ...
        if dateInput.month == 1:
            ...
            ratio = np.interp(dfdays, xp, [0, 1])
            fileOutput = (
                fileInput[dateInput.month - 2, :, :] * (1 - ratio)
                + fileInput[dateInput.month - 1, :, :] * ratio
            )
    return fileOutput
...
def getbasinMap():
    ...
    n_aus = np.argwhere((lonc >= 90) & (lonc < 160)
                        & (latc < 0) & (latc >= -45))
    n_spc = np.argwhere((lonc >= 160) & (lonc < 240)
                        & (latc < 0) & (latc >= -45))
    n_ni = np.argwhere((lonc < 100) & (latc >= 0) & (latc <= 45))
    n_wnp = np.argwhere((lonc >= 100) & (lonc < 180)
                        & (latc >= 0) & (latc <= 45))
    n_enp = np.argwhere(((lonc >= 180) & (lonc < 235)
                        & (latc >= 0) & (latc <= 45)))
    ...
    a = np.arange(xcenter.shape[0] * ycenter.shape[0]).reshape(
        xcenter.shape[0], ycenter.shape[0]
    )
    basinMap[a == 1716] = 2
    basinMap[a == 1715] = 2
    basinMap[a == 1714] = 2
    ...
    for iN in range(time.shape[0] - 1):
        ...
        if (lon[iN + 1] == lon[iN + 1]) and (delt > 0):
            ...
            for iM in range(1, delt, 1):
                ...
                lonInt.append((1.0 - iM * inv) *
                              lon[iN] + iM * inv * lon[iN + 1])
                latInt.append((1.0 - iM * inv) *
                              lat[iN] + iM * inv * lat[iN + 1])
        else:
            timeInt.append(datetime(1800, 1, 1, 0, 0))
            lonInt.append(float("nan"))
            latInt.append(float("nan"))

    speed = np.full(lon.shape[0], np.nan, dtype=float)
    sdir = np.full(lon.shape[0], np.nan, dtype=float)
    count = 0
    for it in time:
        ...
        speed[count] = distance * 1000.0 / (nup - ndn) / 60 / 60  # m/s
        count += 1
