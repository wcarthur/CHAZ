...
class Vregression1():
    ...
    def __init__(self, n1, n2):
        self.y = np.full([n1], np.nan, dtype=float)
        self.x = np.full([n1, n2], np.nan, dtype=float)
...
def writePredictors(bt, leadingTime, ty1, ty2, py1, py2, predictors, fstType):
    ...
    TimeDepends = ['dThetaEMean', 'T200Mean', 'rhMean',
                   'rh500_300Mean', 'div200Mean', 'T100Mean', 'dThetaEsMean']
            ...
            for it in range(lT, bt.StormLon[:, iS].shape[0], 1):
                ...
                if bt.StormLon[it, iS] == bt.StormLon[it, iS]:
                    ...
                    varRegress.y[count1] = bt.StormMwspd[it, iS] - \
                        bt.StormMwspd[it-lT, iS]
                    count2 = 0
                    # the independent variables are from the initial time, so it is it - lT
                    for var in predictors:
                        if var in 'SHRD':
                            varRegress.x[count1, count2] = nanmean(
                                np.sqrt(bt.UShearMean[it-lT:it+1, iS]**2+bt.VShearMean[it-lT:it+1, iS]**2))
                        elif var in 'SHRD_I':
                            varRegress.x[count1, count2] = np.sqrt(
                                bt.UShearMean[it-lT, iS]**2+bt.VShearMean[it-lT, iS]**2)
                        elif var in 'dPIwspd':
                            varRegress.x[count1, count2] = nanmean(
                                bt.PIwspdMean[it-lT:it+1, iS])-bt.StormMwspd[it-lT, iS]
                        elif var in 'dPIslp':
                            varRegress.x[count1, count2] = nanmean(
                                bt.PIslp[it-lT:it+1, iS])-bt.StormMslp[it-lT, iS]
                        elif var in TimeDepends:
                            varRegress.x[count1, count2] = nanmean(
                                getattr(bt, var)[it-lT:it+1, iS])
                        elif var in 'landmaskMean':
                            varRegress.x[count1, count2] = getattr(
                                bt, var)[it, iS]-getattr(bt, var)[it-lT, iS]
                        elif var in 'StormMwspd':
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'StormMwspd')[it-lT, iS])
                        elif var in 'StormMwspd2':
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'StormMwspd')[it-lT, iS])**2
                        elif var in 'StormMwspd3':
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'StormMwspd')[it-lT, iS])**3
                        elif var in 'MPI':
                            varRegress.x[count1, count2] = nanmean(
                                bt.PIwspdMean[it-lT:it+1, iS])
                        elif var in 'MPI2':
                            varRegress.x[count1, count2] = nanmean(
                                bt.PIwspdMean[it-lT:it+1, iS])**2
                        elif var in 'dPIwspd2':
                            varRegress.x[count1, count2] = (
                                nanmean(bt.PIwspdMean[it-lT:it+1, iS])-bt.StormMwspd[it-lT, iS])**2
                        elif var in 'dPIwspd3':
                            varRegress.x[count1, count2] = (
                                nanmean(bt.PIwspdMean[it-lT:it+1, iS])-bt.StormMwspd[it-lT, iS])**3
                        elif 'dVdt2' in var:
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'dVdt')[it-lT, iS])**2
                        elif 'dVdt3' in var:
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'dVdt')[it-lT, iS])**3
                        elif var in 'landmaskMean':
                            varRegress.x[count1, count2] = getattr(
                                bt, var)[it, iS]-getattr(bt, var)[it-lT, iS]
                        else:
                            varRegress.x[count1, count2] = getattr(bt, var)[
                                it-lT, iS]
                        count2 += 1
                    count1 += 1

        iS += 1
        if iS > bt.StormYear.shape[0]-1:
            break
            ...
            for it in range(lT, bt.StormLon[:, iS].shape[0], 1):
                if ((bt.StormLon[it, iS] == bt.StormLon[it, iS]) & (bt.landmaskMean[it, iS] != 0)):
                    if (fstType == 'Vmax'):
                        ...
                        varRegress.y[count1] = bt.StormMwspd[it,
                                                             iS]-bt.StormMwspd[it-lT, iS]
                    count2 = 0
                    for var in predictors:
                        if var in 'SHRD':
                            varRegress.x[count1, count2] = nanmean(
                ...
                if ((bt.StormLon[it, iS] == bt.StormLon[it, iS]) & ((bt.landmaskMean[it, iS] > -0.5) | (bt.landmaskMean[it-lT, iS] > -0.5))):
                    ...
                    for var in predictors:
                        if var in 'SHRD':
                            ...
                            if it-lT >= 2:
                                varRegress.x[count1, count2] = (
                                    getattr(bt, 'StormMwspd')[it-lT-2, iS])**2
                            else:
                                varRegress.x[count1, count2] = (
                                    getattr(bt, 'StormMwspd')[it, iS])**2
                        else:
                            varRegress.x[count1, count2] = getattr(bt, var)[
                                it-lT, iS]
        ...
        if ((int(bt.StormYear[iS]) >= ty1) & (int(bt.StormYear[iS]) < ty2)):
            for it in range(lT, bt.StormLon[:, iS].shape[0], 1):
                ...
                if ((bt.StormLon[it, iS] == bt.StormLon[it, iS]) & ((bt.landmaskMean[it, iS] <= -0.5) & (bt.landmaskMean[it-lT, iS] <= -0.5))):
                    ...
                    count2 = 0
                    var = 'landmaskMean'
                    # getattr(bt,var)[it,iS]-getattr(bt,var)[it-lT,iS]
                    dldMask = 1.0
                    for var in predictors:
                        if var == 'SHRD':
                            varRegress.x[count1, count2] = nanmean(
                            ...
                            varRegress.x[count1, count2] = getattr(bt, 'StormMwspd')[
                                it-lT, iS]*dldMask
                        elif var == 'dPIwspd2':
                            varRegress.x[count1, count2] = (
                                nanmean(bt.PIwspdMean[it-lT:it+1, iS])-bt.StormMwspd[it-lT, iS])**2
                            ...
                            varRegress.x[count1, count2] = getattr(
                                bt, 'dVdt')[it-lT, iS]*dldMask
                        elif var == 'dPIwspdL':
                            varRegress.x[count1, count2] = (
                                nanmean(bt.PIwspdMean[it-lT:it+1, iS])-bt.StormMwspd[it-lT, iS])*dldMask
                        elif var == 'SHRDL':
                            varRegress.x[count1, count2] = nanmean(np.sqrt(
                                bt.UShearMean[it-lT:it+1, iS]**2+bt.VShearMean[it-lT:it+1, iS]**2))*dldMask
                        elif var == 'rhMeanL':
                            varRegress.x[count1, count2] = nanmean(
                                getattr(bt, 'rhMean')[it-lT:it+1, iS])*dldMask
                        elif var == 'dPIwspd2L':
                            varRegress.x[count1, count2] = dldMask*(
                                nanmean(bt.PIwspdMean[it-lT:it+1, iS])-bt.StormMwspd[it-lT, iS])**2
                        elif var == 'dPIwspd3L':
                            varRegress.x[count1, count2] = dldMask*(
                                nanmean(bt.PIwspdMean[it-lT:it+1, iS])-bt.StormMwspd[it-lT, iS])**3
                            ...
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'trSpeed')[it-lT, iS])*dldMask
                        else:
                            varRegress.x[count1, count2] = getattr(bt, var)[
                                it-lT, iS]
                        count2 += 1
...
def calPrediction_bt(bt, leadingTime, py1, py2, predictors, pError, countE, coeffFile, meanFile):
        ...
        if ((int(bt.StormYear[iS]) >= py1) & (int(bt.StormYear[iS]) < py2)):
            for it in range(lT, bt.StormLon[:, iS].shape[0], 1):
                if bt.StormLon[it, iS] == bt.StormLon[it, iS]:
                    ...
                    varRegress1.y[count1] = bt.StormMwspd[it,
                                                          iS]-bt.StormMwspd[it-lT, iS]
                    count2 = 0
                    for var in predictors:
                        if var in 'SHRD':
                            varRegress1.x[count1, count2] = nanmean(
                                np.sqrt(bt.UShearMean[it-lT:it+1, iS]**2+bt.VShearMean[it-lT:it+1, iS]**2))
                        elif var in 'SHRD_I':
                            varRegress1.x[count1, count2] = np.sqrt(
                                bt.UShearMean[it-lT, iS]**2+bt.VShearMean[it-lT, iS]**2)
                        elif var in 'dPIwspd':
                            varRegress1.x[count1, count2] = nanmean(
                                bt.PIwspd[it-lT:it+1, iS])-bt.StormMwspd[it-lT, iS]
                        elif var in 'dPIslp':
                            varRegress1.x[count1, count2] = nanmean(
                                bt.PIslp[it-lT:it+1, iS])-bt.StormMslp[it-lT, iS]
                        elif var in TimeDepends:
                            ...
                            varRegress1.x[count1, count2] = getattr(bt, var)[
                                it-lT, iS]
                        else:
                            varRegress1.x[count1, count2] = getattr(bt, var)[
                                it-lT, iS]
    ...
    for iv in range(1, n2+1, 1):
        X2[:, iv] = (X2[:, iv]-float(lm[iv+2])) / \
            float(lm[iv+2+len(predictors)])

    Y2 = float(ll[1])
    for iv in range(1, n2+1, 1):
        Y2 = Y2 + X2[:, iv]*float(ll[iv+1])

    Y2 = (Y2*float(lm[2])) + float(lm[1])
    Y2 = Y2 + Y1
...
def calMultiRegression(bt, sf, of, leadingTime, ty1, ty2, py1, py2, predictors, fstType, pError, sError, oError, bError, countE):
    ...
    varRegress = Vregression1(n1, n2)
    varRegress1 = Vregression1(n1, n2)
    TimeDepends = ['dThetaMean', 'T200Mean',
                   'rhMean', 'rh500_300Mean', 'div200Mean']
            ...
            for it in range(lT, bt.StormLon[:, iS].shape[0], 1):
                if bt.StormLon[it, iS] == bt.StormLon[it, iS]:
                    ...
                    for var in predictors:
                        if var in 'SHRD':
                            varRegress.x[count1, count2] = nanmean(
                                np.sqrt(bt.UShearMean[it-lT:it, iS]**2+bt.VShearMean[it-lT:it, iS]**2))
                        elif var in 'SHRD_I':
                            varRegress.x[count1, count2] = np.sqrt(
                                bt.UShearMean[it-lT, iS]**2+bt.VShearMean[it-lT, iS]**2)
                        elif var in 'dPIwspd':
                            varRegress.x[count1, count2] = nanmean(
                                bt.PIwspd[it-lT:it, iS])-bt.StormMwspd[it-lT, iS]
                        elif var in TimeDepends:
                            varRegress.x[count1, count2] = nanmean(
                                getattr(bt, var)[it-lT:it, iS])
                        else:
                            varRegress.x[count1, count2] = getattr(bt, var)[
                                it-lT, iS]
                        count2 += 1
    ...
    plt.savefig('scatter_'+fstType+'_' +
                int2str(leadingTime, 3)+'hr.png', dpi=300)
    plt.clf()
...
def singleStorm(id, year, name, of, sf, bt, coeffFile, meanFile, predictors, TimeDepends):
    ...
    for iS in range(0, sf.StormTime.shape[1], 2):
        ...
        if sf.StormTime[index, iS]:
            time1 = sf.fstT[index, iS, :][(sf.fstT[index, iS, :] < 9999.0) & (
                sf.VMAX[index, iS, :] == sf.VMAX[index, iS, :])]
            v1 = sf.VMAX[index, iS, :][(sf.fstT[index, iS, :] < 9999.0) & (
                sf.VMAX[index, iS, :] == sf.VMAX[index, iS, :])]
            v1[v1 >= 300.] = float('nan')
            time2 = np.array([])
            ...
            for iv1 in range(0, v1.shape[0], 1):
                time2 = np.append(
                    time2, sf.StormTime[index, iS] + timedelta(hours=time1[iv1]))
    ...
    for iS in range(0, of.StormTime.shape[1], 2):
        if of.StormTime[index, iS]:
            time1 = of.fstT[index, iS, :][(of.fstT[index, iS, :] < 9999.0) & (
                of.VMAX[index, iS, :] == of.VMAX[index, iS, :])]
            if time1.any():
                v1 = of.VMAX[index, iS, :][(of.fstT[index, iS, :] < 9999.0) & (
                    of.VMAX[index, iS, :] == of.VMAX[index, iS, :])]
                v1[v1 >= 300.] = float('nan')
                time2 = np.array([])
                for iv1 in range(0, v1.shape[0], 1):
                    time2 = np.append(
                        time2, of.StormTime[index, iS] + timedelta(hours=time1[iv1]))
    ...
    fstperiod = list(range(12, 132, 12))
    ...
    for it in range(0, bt.StormMwspd.shape[0], 2):
        if (((it+lT) <= bt.StormMwspd.shape[0]) and (bt.StormMwspd[it, index] == bt.StormMwspd[it, index])):
            h1, v1 = getPrediction(bt, meanX, meanY, stdX, stdY, it,
                                   index, fstperiod, coeffFile, predictors, TimeDepends)
            plt.plot(h1, v1, 'r-')

    plt.plot(bt.Time[:, index], bt.StormMwspd[:, index], 'k-')
    days = DayLocator(interval=3)
    hours = HourLocator(list(range(0, 24, 6)))
    ...
    plt.title('Hurricane '+name+' ('+str(year) +
              '): blue-SHIPS, gray-OFCL, red', fontsize=8)
        ...
        for it in range(0, bt.StormMwspd.shape[0], 2):
            if (it+lT) < bt.StormMwspd.shape[0]:
                if (bt.StormLon[it, index] == bt.StormLon[it, index]) and (bt.StormLon[it+lT, index] == bt.StormLon[it+lT, index]):
                    count2 = 0
                    varRegress.y[count1] = bt.StormMwspd[it +
                                                         lT, index] - bt.StormMwspd[it, index]
                    for var in predictors:
                        if var in 'SHRD':
                            varRegress.x[count1, count2] = nanmean(np.sqrt(
                                bt.UShearMean[it-lT:it+1, index]**2+bt.VShearMean[it-lT:it+1, index]**2))
                        elif var in 'SHRD_I':
                            varRegress.x[count1, count2] = np.sqrt(
                                bt.UShearMean[it-lT, index]**2+bt.VShearMean[it-lT, index]**2)
                        elif var in 'dPIwspd':
                            varRegress.x[count1, count2] = nanmean(
                                bt.PIwspdMean[it-lT:it+1, index])-bt.StormMwspd[it-lT, index]
                        elif var in 'dPIslp':
                            varRegress.x[count1, count2] = nanmean(
                                bt.PIslp[it-lT:it+1, index])-bt.StormMslp[it-lT, index]
                        elif var in TimeDepends:
                            varRegress.x[count1, count2] = nanmean(
                                getattr(bt, var)[it-lT:it+1, index])
                        elif var in 'landmaskMean':
                            varRegress.x[count1, count2] = getattr(
                                bt, var)[it, index]-getattr(bt, var)[it-lT, index]
                        elif var in 'StormMwspd':
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'StormMwspd')[it-lT, index])
                        elif var in 'StormMwspd2':
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'StormMwspd')[it-lT, index])**2
                        elif var in 'StormMwspd3':
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'StormMwspd')[it-lT, index])**3
                        elif var in 'MPI':
                            varRegress.x[count1, count2] = nanmean(
                                bt.PIwspdMean[it-lT:it+1, index])
                        elif var in 'MPI2':
                            varRegress.x[count1, count2] = nanmean(
                                bt.PIwspdMean[it-lT:it+1, index])**2
                        elif var in 'dPIwspd2':
                            varRegress.x[count1, count2] = (
                                nanmean(bt.PIwspdMean[it-lT:it+1, index])-bt.StormMwspd[it-lT, index])**2
                        elif var in 'dPIwspd3':
                            varRegress.x[count1, count2] = (
                                nanmean(bt.PIwspdMean[it-lT:it+1, index])-bt.StormMwspd[it-lT, index])**3
                        elif 'dVdt2' in var:
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'dVdt')[it-lT, index])**2
                        elif 'dVdt3' in var:
                            varRegress.x[count1, count2] = (
                                getattr(bt, 'dVdt')[it-lT, index])**3
                        elif var in 'pre_V0':
                            if it-lT >= 2:
                                varRegress.x[count1, count2] = getattr(
                                    bt, 'StormMwspd')[it-lT-2, index]
                            else:
                                varRegress.x[count1, count2] = getattr(
                                    bt, 'StormMwspd')[it, index]
                        elif var in 'pre_V02':
                            if it-lT >= 2:
                                varRegress.x[count1, count2] = (
                                    getattr(bt, 'StormMwspd')[it-lT-2, index])**2
                            else:
                                varRegress.x[count1, count2] = (
                                    getattr(bt, 'StormMwspd')[it, index])**2
                        else:
                            varRegress.x[count1, count2] = getattr(bt, var)[
                                it-lT, index]
                        count2 += 1
...
def getPrediction(bt, meanX, meanY, stdX, stdY, it, index, fstperiod, coeffFile, predictors, TimeDepends):
    ...
    for ih in fstperiod:
        ...
        for var in predictors:
            ...
            if var in 'SHRD':
                rvar = nanmean(
                    np.sqrt(bt.UShearMean[it:it+lT, index]**2+bt.VShearMean[it:it+lT, index]**2))
            elif var in 'dPIwspd':
                rvar = nanmean(bt.PIwspd[it:it+lT, index]
                               )-bt.StormMwspd[it, index]
            elif var in TimeDepends:
...
def getPrediction_oneTime(bt, meanX, meanY, stdX, stdY, it, index, ih, coeffFile, predictors, TimeDepends):
    ...
    for var1 in predictors:
        if var1 in 'SHRD':
            ...
            rvar = np.sqrt(bt.UShearMean[it, index]
                           ** 2+bt.VShearMean[it, index]**2)
...
def getPrediction_v0input(bt, meanX, meanY, stdX, stdY, it, index, fstperiod, coeffFile, predictors, TimeDepends, v0, dvdt):
    ...
    for ih in fstperiod:
        ...
        for var in predictors:
            # print meanX.shape
            if var in 'SHRD':
                rvar = nanmean(np.sqrt(
                    bt.UShearMean[it:it+lT+1, index]**2+bt.VShearMean[it:it+lT+1, index]**2))
            elif var in 'dPIwspd':
                ...
                rvar = getattr(bt, var)[it+lT+1, index] - \
                    getattr(bt, var)[it, index]
            elif var in 'StormMwspd2':
            ...
            rvar = (rvar-meanX[ih/12-1, count2])/stdX[ih/12-1, count2]
            dy = dy + rvar*float(ll[count2+2])
            # print count2,var, rvar*float(ll[count2+2])
            count2 = count2 + 1
        dy = (dy*stdY[ih/12-1])+meanY[ih/12-1]
        v1.append(v0+dy)
